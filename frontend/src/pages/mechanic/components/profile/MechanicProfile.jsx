 ...
 * 
 * Displays and allows editing of mechanic profile info from the API.
 * API GET /api/auth/users/:id/profile returns:
 *   { name, phone, location: { lng, lat }, working_hours, usertype }
 */
export const MechanicProfile = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        if (user?.id) {
          ...
          // If profile has location, reverse geocode it for display
          if (profileData.location?.lat && profileData.location?.lng) {
            reverseGeocode(profileData.location.lat, profileData.location.lng);
          }
        }
      } catch (err) {
  ...
  const reverseGeocode = async (lat, lng) => {
    ...
    try {
      ...
      if (data && data.display_name) {
        ...
        setLocationAddress(locationString || data.display_name);
      }
    } catch (err) {
  ...
  /**
   * Use current GPS position to update location
   */
  const handleUseCurrentLocation = () => {
    if (gpsPosition) {
      ...
      setEditedProfile(prev => ({ ...prev, location: newLocation }));
      reverseGeocode(gpsPosition.lat, gpsPosition.lng);
    }
  };

  const handleEdit = () => {
    setEditedProfile({ ...profile, location: { ...profile.location } });
    setIsEditing(true);
  ...
  const handleSave = async () => {
    ...
    try {
      await authService.updateProfileById(user.id, {
        ...
        working_hours: editedProfile.working_hours,
        mechanic_lat: editedProfile.location?.lat,
        mechanic_lng: editedProfile.location?.lng,
      });
      setProfile({ ...editedProfile, location: { ...editedProfile.location } });
      setIsEditing(false);
      setSaveSuccess(true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <Card className="overflow-hidden rounded-none shadow-sm">
        {/* Top navigation bar */}
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-4 pt-4 pb-0">
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900 capitalize">{profile.usertype || 'mechanic'}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div>
              ...
              {isEditing ? (
                ...
                <p className="font-medium text-gray-900">{profile.working_hours || 'Not set'}</p>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            ...
            {locationAddress && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                <p className="text-sm text-gray-600 mb-1">📍 Approximate Address</p>
                <p className="font-medium text-gray-900 text-sm">{locationAddress}</p>
              </div>
            )}
              ...
              <div>
                ...
                {isEditing ? (
                  <Input
                    type="number"
                    step="any"
                    value={editedProfile.location?.lat ?? ''}
                    onChange={(e) => handleLocationChange('lat', e.target.value)}
                    placeholder="11.5619"
                  />
                ) : (
                  <p className="font-medium text-gray-900">
                    {profile.location?.lat ?? 'Not set'}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  <Input
                    type="number"
                    step="any"
                    value={editedProfile.location?.lng ?? ''}
                    onChange={(e) => handleLocationChange('lng', e.target.value)}
                    placeholder="104.927"
                  />
                ) : (
                  <p className="font-medium text-gray-900">
                    {profile.location?.lng ?? 'Not set'}
                  </p>
                )}
              </div>
            </div>
            ...
            {isEditing && (
              <div className="flex gap-2">
                ...
                >
                  ...
                  {isLoadingLocation ? 'Getting location...' : 'Use Current GPS'}
                </Button>
              </div>
            )}
