...
const History = () => {
  ...
  const handleCancelRequest = async (requestId) => {
    if (!requestId || actionState.isLoading) return;
    ...
    try {
      ...
      setSelectedHistory((prev) =>
        prev && prev.id === requestId
          ? { ...prev, status: 'cancelled', canCancel: false }
          : prev
      );
  ...
  const baseHistory = Array.isArray(historyData) ? historyData : [];
  ...
  const totalSpent = baseHistory
    ...
    .reduce((sum, item) => sum + (Number(item.totalAmount ?? item.price) || 0), 0);
  const averageRating = baseHistory.length
    ? (baseHistory.reduce((sum, item) => sum + (Number(item.rating) || 0), 0) / baseHistory.length).toFixed(1)
    : '—';
    ...
    <div className="flex items-center justify-center py-24">
      <div className="text-center">
        <motion.div
          ...
          className="mx-auto mb-6 w-16 h-16 border-4 border-primary/20 border-t-primary rounded-full"
        />
        <p className="text-gray-600 font-medium">Fetching your history…</p>
      </div>
    </div>
  );

  const renderEmptyState = () => (
    <motion.div className="text-center py-24 bg-white rounded-3xl shadow-sm" {...fadeIn}>
      <motion.div
      ...
      <h3 className="text-2xl font-bold text-gray-900 mb-3">No Service History</h3>
      <p className="text-lg text-gray-600 max-w-sm mx-auto">Your completed services will appear here once available.</p>
    </motion.div>
  );

  return (
    <div className="h-screen flex flex-col bg-gradient-to-b from-gray-50 to-gray-100">
      <motion.div className="bg-white border-b border-gray-100 px-4 md:px-6 py-4 md:py-6 z-30 shadow-sm flex-shrink-0" {...fadeIn}>
            ...
            {baseHistory.length > 0 && (
              <>
                ...
                <Badge variant="secondary" className="font-semibold text-xs">
                  {baseHistory.length} {baseHistory.length === 1 ? 'service' : 'services'}
                </Badge>
              </>
            )}
          </motion.div>
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-6 py-5 md:py-7 pb-20 md:pb-8">
        <div className="max-w-4xl mx-auto">
          ...
          {!isLoading && baseHistory.length === 0 && renderEmptyState()}
          ...
          {!isLoading && baseHistory.length > 0 && (
            <>
              <div className="grid grid-cols-2 gap-3 mb-6">
                ...
                >
                  <p className={cn('font-bold text-sm', activeTab === 'completed' ? 'text-white' : 'text-gray-900')}>
                    Completed
                  </p>
                  <p className={cn('text-xs', activeTab === 'completed' ? 'text-blue-100' : 'text-gray-600')}>
                    {completedCount} done
                  </p>
                </button>
              </div>
              ...
              <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6" variants={staggerContainer} initial="initial" animate="animate">
                <motion.div className="bg-white rounded-2xl p-4 shadow-sm" variants={staggerItem}>
                ...
                <motion.div className="bg-white rounded-2xl p-4 shadow-sm" variants={staggerItem}>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{baseHistory.length}</p>
                </motion.div>
              </motion.div>

              <motion.div
                key={`history-list-${activeTab}`}
                className="space-y-4"
              ...
              >
                {filteredHistory.length === 0 ? (
                  ...
                  filteredHistory.map((item, index) => {
                    const cardKey = item.id ?? `history-${item.serviceDate ?? 'unknown'}-${index}`;
                    const totalDisplay = formatCurrency(item.totalAmount ?? item.price);
                    ...
                    return (
                      <motion.div key={`${activeTab}-${cardKey}`} variants={staggerItem} initial={false}>
                        <motion.div whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.2 }}>
                          <Card className="shadow-md hover:shadow-xl transition-shadow cursor-pointer border-0 overflow-hidden" onClick={() => handleViewDetails(item)}>
                            <CardContent className="p-5 md:p-6 relative">
                              <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-primary/5 to-transparent rounded-bl-full" />
                              <div className="flex items-start justify-between mb-4 relative">
                                <div className="flex items-start gap-3 md:gap-4 flex-1 min-w-0">
                                  <motion.div className="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-green-600 to-green-700 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg" whileHover={{ rotate: 5, scale: 1.05 }}>
                                    <Wrench className="w-7 h-7 md:w-8 md:h-8 text-white" />
                                  </motion.div>
                                  <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-gray-900 text-lg md:text-xl mb-1.5">{item.mechanicName}</h3>
                                    <p className="text-base md:text-lg text-gray-700 font-semibold mb-2">{item.serviceType}</p>
                                  </div>
                                </div>
                                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.1 + index * 0.05, type: 'spring' }}>
                              ...
                              <div className="flex flex-wrap gap-4 items-center justify-between pt-4 border-t-2 border-gray-100">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-2xl md:text-3xl font-bold text-green-600">${totalDisplay}</span>
                                  </div>
                                </div>
                                <div className="flex items-center gap-3">
                                  <div className="flex items-center gap-1.5 bg-yellow-50 px-3 py-2 rounded-xl">
                                    <Star className="w-5 h-5 fill-yellow-500 text-yellow-500" />
                                    <span className="font-bold text-gray-900 text-base">{ratingLabel}</span>
                                  </div>
      ...
      <AnimatePresence>
        {showDetails && selectedHistory && (
          <motion.div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center md:justify-center" onClick={() => setShowDetails(false)} {...backdropFade}>
            <motion.div
            ...
            >
              <div className="sticky top-0 bg-white border-b border-gray-100 px-5 md:px-6 py-5 md:py-6 rounded-t-3xl md:rounded-t-2xl z-10 shadow-sm">
                <div className="flex items-center justify-between">
                  <motion.h2 className="text-xl md:text-2xl font-bold text-gray-900 flex items-center gap-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                    <FileText className="w-6 h-6 text-primary" />
                    Service Details
                  </motion.h2>
                  <motion.div whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }}>
                    <Button variant="ghost" size="icon" onClick={() => setShowDetails(false)} className="hover:bg-gray-100">
                      <X className="w-5 h-5" />
                    </Button>
                  </motion.div>
                </div>
              ...
              <div className="px-5 md:px-6 py-5 md:py-6 space-y-5">
                <div className="bg-gray-50 rounded-2xl p-4">
                  ...
                  {selectedHistory.mechanicPhone && (
                    <Button variant="secondary" size="sm" className="w-full" onClick={() => window.open(`tel:${selectedHistory.mechanicPhone}`)}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Badge className={cn('text-sm font-semibold border', getStatusColor(selectedHistory.status))}>
                      ...
                      {formatStatus(selectedHistory.status)}
                    </Badge>
                  </div>
                  {selectedHistory.notes && (
                    <div>
                      <label className="text-sm font-semibold text-gray-600 mb-1 block">Notes</label>
                      <p className="text-sm text-gray-700 leading-relaxed bg-blue-50 p-3 rounded-xl">{selectedHistory.notes}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-gray-50 rounded-2xl p-4">
                  ...
                  <div className="space-y-2 mb-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-semibold text-gray-900">${formatCurrency(selectedHistory.tripFee)}</span>
                      </div>
                    </div>
                    <div className="flex justify-between text-sm">
                      ...
                      <div className="flex items-center gap-1">
                        <DollarSign className="w-4 h-4 text-gray-500" />
                        <span className="font-semibold text-gray-900">${formatCurrency(selectedHistory.serviceFee)}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-2xl font-bold text-green-600">${formatCurrency(selectedHistory.totalAmount)}</span>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          ...
                          className={cn(
                            'w-5 h-5',
                            i < Number(selectedHistory.rating || 0)
                ...
                {selectedHistory.canRespondToPriceChange && (
                  <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 space-y-3">
                    <p className="text-sm text-gray-700 font-medium">
                      The mechanic proposed a new price. Do you accept the change?
                    </p>
                    <div className="flex flex-col md:flex-row gap-3">
                      <Button variant="default" className="flex-1" onClick={() => acceptProposedPrice(selectedHistory.id)}>
                        Accept Proposed Price
                      </Button>
                      <Button variant="secondary" className="flex-1" onClick={() => declineProposedPrice(selectedHistory.id)}>
                        Decline
                      </Button>
                    </div>
                  </div>
                ...
                {selectedHistory.canCancel && (
                  <div className="bg-red-50 border border-red-100 rounded-2xl p-4 space-y-3">
                    ...
                    <Button
                      variant="destructive"
                      className="w-full"
                      onClick={() => handleCancelRequest(selectedHistory.id)}
                      disabled={actionState.isLoading && actionState.type === 'cancel'}
                    >
                      {actionState.isLoading && actionState.type === 'cancel' ? 'Cancelling…' : 'Cancel Request'}
                    </Button>
                  </div>
                )}
              </div>
